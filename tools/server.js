...
const soundData = require("./sound-data-2016-09.json").concat(require("./sound-data-extra.json"));
...
function getHearthheadSoundUrl(audioFileName) {
    if (audioFileName.indexOf("Play.ogg") >= 0) {
        ...
        return;
    } else if (audioFileName.indexOf("Death.ogg") >= 0) {
        var minionName = audioFileName.split("Death.ogg")[0].replace(/'/g, "&#039;");
        var data = soundData.filter(function (data) {
            return data.name === minionName;
        })[0];
        if (data) {
            return "http://" + data.deathSoundUrl;
        }
        return;
